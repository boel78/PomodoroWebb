...
export default function Pomodoro() {
          ...
          if (sessionData.success) {
            ...
            try {
              const userSessionResponse = await fetch(
                `http://localhost:5239/api/Session/getSessionsByUsername/${user.userName}`,
                {
                  method: "GET",
                }
              );
  ...
  const handleNewSession = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const [preferedhours, preferedminutes, preferedseconds] =
      user.preferredPomodoro.split(":").map(Number);
    ...
    if (user?.algoritmsetting === "longer_break") {
      setShowExtraTimeText(
        extraTime && !isNaN(extraTime)
          ? `You will have ${formatTime(
              Math.round(extraTime)
            )} per break extra time for breaks`
          : ""
      );
    } else {
      setShowExtraTimeText(
        extraTime && !isNaN(extraTime)
          ? `You will have ${formatTime(
              Math.round(extraTime ?? 0)
            )} per pomodoro extra time for work`
          : ""
      );
    }
  };
  ...
  useEffect(() => {
    if (user) {
      //Achievements kod
      //Lägg till first timer
      if (
        !isSessionCancelledRef.current &&
        userSessions &&
  ...
  return (
    <div className="bg-tomato-50 flex flex-col-reverse justify-end md:grid md:grid-cols-4 md:h-dvh overflow-y-auto md:overflow-hidden h-dvh">
      ...
      <div className="col-span-1 flex flex-col md:items-center shadow-lg bg-tomato-100 px-3 overflow-scroll mt-auto md:pt-12 h-dvh">
        <h3
          className="font-semibold text-3xl text-tomato-700 text-center py-6"
          onClick={handleSetSessionVisible}
        >
          Previous sessions
        </h3>
        <div
          className={`flex flex-col gap-4 transition-all duration-300 overflow-scroll ${
      ...
      <div className="col-span-3 flex flex-col items-center md:pb-8 md:h-full mt-20 overflow-y-auto">
        {timerIsActive ? (
          <div>
            {showCancelWindow && (
              <ConfirmCancel
                setShowCancelWindow={setShowCancelWindow}
                handleSuccesfullSession={handleSuccesfullSession}
                isSessionRef={isSessionCancelledRef}
              />
            )}
            <div className="flex flex-col items-center gap-10">
              <div className="flex items-center gap-3">
                {showRemainingTime ? (
                  <>
                    ...
                    <span onClick={() => setShowRemainingTime(false)}>
              ...
              <div className="flex flex-col items-center gap-5">
                ...
                <Button
                  variant={"outline"}
                  className="bg-tomato-700 text-tomato-50"
                  onClick={() => setShowCancelWindow(!showCancelWindow)}
                >
                  Cancel
                </Button>
                <TaskList
          ...
          <div className="flex flex-col items-center gap-10 overflow-y-auto w-full">
            <h2 className="font-semibold text-tomato-700 text-3xl pt-6 pb-12">
